...
import { Send, Clock, Mail, Calendar, Settings, Sparkles, AlertCircle, BookOpen, Mic, MicOff, Volume2, VolumeX, Check, Plus, Search, X, Loader2, Moon, Sun } from 'lucide-react';
...
const Dashboard = () => {
  ...
  const [chatHistory, setChatHistory] = useState([
    { role: 'ai', text: "Namaste! My memory is active and I'm ready to help you." }
  ]);
  const [emails] = useState([
    { id: 1, sender: 'Prof. Sharma', subject: 'Re: Paper Submission Timeline', time: '10:45 AM', tag: 'Action Required', snippet: 'Can you please outline the timeline for the RL section?' }
  ]);
  ...
  const loadSettingsAndData = () => {
    ...
    fetch('http://localhost:8000/api/calendar')
      .then(res => res.json()).then(data => setEvents(data.events || [])).catch(e => console.error(e));

    fetch('http://localhost:8000/api/research')
      .then(res => res.json()).then(data => setResearch(data.papers || [])).catch(e => console.error(e));
  };
  ...
  const saveSettings = async (e) => {
    ...
    // reload research to get new topic
    loadSettingsAndData();
  };
  ...
  const handleSend = async (overrideText = null) => {
    ...
    setQuery('');
    setWebSearchEnabled(false); // reset toggle
    setIsTyping(true);
    ...
    try {
      const response = await fetch('http://localhost:8000/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ message: text, require_web_search: webSearchEnabled })
      });
      const data = await response.json();
      setChatHistory(prev => [...prev, { role: 'ai', text: data.reply }]);
      speak(data.reply);
    } catch(e) {
      setChatHistory(prev => [...prev, { role: 'ai', text: "Ollama is disconnected or taking too long." }]);
    }
    setIsTyping(false);
  };
  ...
  return (
    <div className="min-h-screen relative flex h-screen overflow-hidden text-primary">
      ...
      {/* Background ambient blobs for super cool Japanese styling */}
      ...
      <motion.div 
        ...
        className="w-[450px] border-r border-theme glass-panel flex flex-col h-full z-10"
      >
          ...
          <section>
            ...
            <div className="space-y-3">
              {events.length === 0 ? <Loader2 size={16} className="animate-spin text-secondary" /> : events.map(ev => (
                <motion.div whileHover={{ scale: 1.02 }} key={ev.id} className="p-4 bg-secondary border border-theme rounded-2xl flex items-start gap-4 shadow-[0_4px_20px_var(--accent-glow)] transition-all cursor-default">
                  <div className={`p-2 rounded-xl ${ev.type === 'meeting' ? 'bg-blue-500/10 text-blue-500' : 'bg-rose-500/10 text-rose-500'}`}>
                    {ev.type === 'meeting' ? <Clock size={18} /> : <AlertCircle size={18} />}
                  </div>
                  <div>
      ...
      <div className="flex-1 flex flex-col relative z-10 bg-black/5 dark:bg-black/20">
          ...
          <AnimatePresence>
            {chatHistory.map((msg, idx) => (
               <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[70%] p-5 text-[15px] leading-relaxed shadow-lg ${msg.role === 'user' ? 'bg-[var(--accent)] text-white rounded-3xl rounded-br-sm font-medium' : 'bg-secondary border border-theme text-primary rounded-3xl rounded-bl-sm font-serif'}`}>
                    {msg.role === 'ai' && <div className="text-[11px] font-sans font-bold mb-2 uppercase tracking-widest flex items-center opacity-70"><Sparkles size={12} className="mr-1"/> Saathi</div>}
                    {msg.text}
                  </div>
               </motion.div>
            ))}
        ...
        <div className="p-8 pt-0">
          <div className="max-w-4xl mx-auto flex gap-3 items-end">
             <div className="flex-1 flex glass-panel rounded-3xl shadow-[0_8px_30px_rgba(0,0,0,0.08)] focus-within:ring-2 ring-[var(--accent)] transition-all overflow-hidden items-center group relative p-1.5 pr-3">
               
               {/* Plus Toggle for Web Search */}
               <button 
                 onClick={() => setWebSearchEnabled(!webSearchEnabled)}
                 className={`p-3 ml-1 rounded-full transition-all flex items-center justify-center shrink-0 ${webSearchEnabled ? 'bg-[var(--accent)] text-white shadow-[0_0_15px_var(--accent-glow)]' : 'text-secondary hover:bg-black/5 dark:hover:bg-white/10'}`}
                 title="Force Web Search"
               >
                 {webSearchEnabled ? <Search size={18} /> : <Plus size={18} />}
               </button>

               <button onClick={toggleListening} className={`p-3 shrink-0 transition-all ${isListening ? 'text-rose-500 animate-pulse' : 'text-secondary hover:text-primary'}`}>
               ...
               <input 
                 ...
                 placeholder={isListening ? "Listening..." : "Ask Saathi anything..."}
                 className="flex-1 bg-transparent border-none outline-none py-3 px-2 text-primary placeholder:text-secondary/70 font-medium w-full min-w-0"
               />
               
               <AnimatePresence>
                 {webSearchEnabled && (
                   <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.8, opacity: 0 }} className="absolute -top-10 left-4 text-[10px] font-bold uppercase tracking-widest bg-[var(--accent)] text-white px-3 py-1.5 rounded-full shadow-lg">
                     Web Scraping Active
                   </motion.div>
                 )}
               </AnimatePresence>
             ...
             <button 
               onClick={() => handleSend()}
               disabled={!query.trim()}
               className="h-14 w-14 shrink-0 bg-[var(--accent)] text-white rounded-full hover:brightness-110 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center shadow-[0_8px_20px_var(--accent-glow)]"
             >
